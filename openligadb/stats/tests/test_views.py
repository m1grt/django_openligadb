...
from openligadb.stats.views import LeagueView, SearchView
from openligadb.stats import forms
...
class SearchViewTest(League):
    def test_league_page_returned_data(self):
        response = self.client.get(
            reverse('search'), {'team_name': 'borussia d'}
        )
        self.assertTrue(
            u'borussia dortmund', response.context['table'][0]['name']
        )
